/**
 * https://github.com/msurguy/backbone.model.computed
 * Written with love by Kelt <kelt@codesleeve.com>
 * Modified by Maksim Surguy
 */
...
(function() {
  ...
  function toCamelCase(_str)
  {
    ...
    while (placement !== -1)
    {
      var capitalizedChar = str.substr(placement + 1, 1).toUpperCase();
      str = str.substr(0, placement) + capitalizedChar + str.substr(placement+2);
   ...
   *     this.get('prop_name');
   */
  function getComputedProperties(model)
  {
  ...
  {
    var properties = getComputedProperties(model);
  ...
  function backboneToJSON(options)
  {
    ...
    var properties = getComputedProperties(this);
